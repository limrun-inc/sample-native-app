...
When the user is satisfied and is done with the dev session, clean up:

```bash
lim ios delete
```

If the user is still iterating in a dev-client or Metro session, leave the simulator running and tell them it is still available.
